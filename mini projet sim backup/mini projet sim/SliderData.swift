...
struct Config {
    ...
    static let colors = [ 0xBA53DE, 0xF469A9, 0xFFFFFF]
                        .shuffled().map { val in Color(netHex: val) }
    static let titleToBeShown = ["Tout frais", "Livraison à domicile", "Paiement facile"]
    static let paragraphToBeShown = [ "Tout ce que vous ferez l'expérience sera absolument frais",
                                      "Tout ce que vous avez à faire est de commander votre plat préféré depuis votre appareil",
                                      "La carte de crédit et le paiement à la livraison sont disponibles pour votre facilité"]
    static let imagesToBeShown = ["walkthough1", "walkthough2", "walkthough3"]
